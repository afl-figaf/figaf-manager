...
| 15h | `cf marketplace -e <offering>` | `faid:provisionServices`, `faid:prepareSpaceServices`, `faid:prepareManagerServices` (`provisionServices` in `faid-apps.js`); `faid:services` for a MISSING row only | cloud | Read-only. Some landscapes name an offering's plans differently (a BTP trial offers `trial` and `proxy` for `credstore`, not `free`/`standard`; 2026-09-14 the Credential Store was silently never created). Before a creation: when the default plan is not offered, the FIRST plan of the module's own allow-list that this landscape offers and does not mark `paid` is used instead - never `proxy` (not in the list), never a paid plan, and never when the person picked the plan themselves (that is refused with what the landscape offers). In `faid:services` the same read fills `availablePlans` so the Setup step 1 dropdown offers only plans that exist here |
