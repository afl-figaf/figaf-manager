...
//   main     :8087  the locally built release (apps/figaf-manager/l3-artifacts,
...
//   remote   :8089  a REMOTE release source: FIGAF_L3_RELEASE_URL points at a
...
const SERVERS = {
  main: {
    port: 8087,
    state: "state.json",
    env: { FIGAF_L3_ARTIFACTS_DIR: path.join(__dirname, "..", "apps", "figaf-manager", "l3-artifacts") },
  },
  failure: {
    port: 8088,
  ...
  remote: {
    ...
    // An empty FIGAF_L3_ARTIFACTS_DIR counts as unset (host.cloud.js), so the
    // developer's own environment cannot turn this server into a local source.
    env: { FIGAF_L3_ARTIFACTS_DIR: "", FIGAF_L3_RELEASE_URL: `http://127.0.0.1:${STORE_PORT}/l3` },
  },
};

// The fixture release store: plain files under e2e/fixtures/store, served as
// a bucket would serve them (GET <url>/l3/index.json, <url>/l3/<v>/<file>).
...
module.exports = async () => {
  ...
  try {
    if (wanted.includes("remote")) {
      if (!fs.existsSync(path.join(STORE_DIR, "l3", "index.json"))) throw new Error(`fixture store missing: run node e2e/tools/make-fixture-store.js`);
