...
|---|---|---|
| `'<app>' is new in release V; this installation runs W ...` (older builds: `unknown app id '<app>' in release W`) | One version per installation (decision 0010): the app was added to the release after the installed version W. The row appeared because the page was reading the latest catalog. | Release panel -> **Update installation to V** (moves the shared backend and every installed app), then **Install** the app. Nothing was deployed by the refusal. |
