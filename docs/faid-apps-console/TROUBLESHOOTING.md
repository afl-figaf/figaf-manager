...
| Setup step 1 "succeeds" but a base service card never turns Ready (a BTP trial subaccount, 2026-09-14: the Credential Store), on a manager build older than 2026-09-14 | The landscape names the offering's plans differently from `packages/core/base-services.js`: a BTP trial offers `credstore` as `trial` and `proxy`, so every `cf create-service credstore free …` failed and the instance was never created — while step 1 still reported success (its service part is non-fatal by design). | Deploy the current manager build: it reads `cf marketplace -e <offering>` first and creates the instance with the landscape's own free plan (`trial`), and the Setup step 1 dropdown only offers plans that exist here. Then Setup step 3 → **Create missing services**. |
| `<offering> offers no plan the manager can use for <name> in this landscape (it offers: …; the manager knows: …)` | The landscape offers none of the plans in the module's allow-list as a free plan — every plan it has is either unknown to the manager (a trial's `proxy` is not a Credential Store) or marked `paid`. Nothing was created; the manager never picks a paid plan by itself. | `cf marketplace -e <offering>` shows the plans. If one of them is the right free plan, ask Figaf to add its name to `plans` in `packages/core/base-services.js` (before `standard`, which must stay last). To use a paid plan, pick it in the Setup step 1 dropdown — a plan you pick yourself is always used as typed. |
| `<offering> does not offer the plan '<plan>' you picked for <name> in this landscape (it offers: …)` | You chose that plan in the Setup step 1 dropdown and the landscape does not have it. A plan picked on purpose is never silently swapped for a smaller one. | Pick one of the plans named in the message, or leave the default and let the manager choose the landscape's free plan. |
