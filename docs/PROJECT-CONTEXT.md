...
> the FAID stream (Arsenii). Kept short and current; history lives in git.
...
figaf-faid decision 0014). Its parts:
...
from Figaf (figaf-faid decision 0004). The manager is the product that ships
...
manager plus the FAID Apps. So every install must work as a virgin-system
...
- **Arsenii Istlentev** — owns the FAID development and delivery process; drives the FAID Apps console work here with Claude.
...
The platform's governance file is figaf-faid `docs/GOVERNANCE.md`. It is
...
- **Frozen identifiers** (figaf-faid decisions 0008, 0009 and 0014): xsappname `figaf-faid`, one XSUAA instance `figaf-faid-xsuaa` for the manager and all apps, scope prefix `FAID`, CF apps `figaf-faid-backend` and `figaf-faid-apps-<app-id>`. Never rename; add.
- **Naming** (figaf-faid decision 0014, 2026-09-07): in UI text and documents write **Figaf Platform**, **Figaf Tool**, **FAID Apps**, **FAID Agents**; in technical names use `faid`, `faid-apps`, `faid-agents`. The old terms L2, L3, L4 are banned everywhere except decision records; nothing carrying them may reach a customer.
...
| Governance, project knowledge, decisions | figaf-faid: `docs/GOVERNANCE.md`, `docs/PROJECT-KNOWLEDGE.md`, `decisions/` |
| Solution documentation of the platform (architecture, status, plan) | figaf-faid: `docs/SOLUTION.md` |
| Release build and publishing (catalog + app zips + `xs-security.json`) | figaf-faid: `release/build.js` writes into `apps/figaf-manager/faid-artifacts/` here (the local source for tests); `release/publish.js` publishes to the release store; the procedure is `release/README.md` there |
| The release store the manager reads | `FIGAF_FAID_RELEASE_URL` in `apps/figaf-manager/manifest.yml`; the reader is `packages/core/release-store.js` (figaf-faid decision 0010) |
...
- Virgin runs #1 to #7 passed in the dev space (record: figaf-faid `docs/d1/RUNBOOK-VIRGIN.md`). Run #7 (2026-09-03) proved the install order of decision 0009: one token, one passcode, one restart, one XSUAA instance.
- The branch `poc/faid-apps-manager` is pushed to GitHub but not merged into master. Merging is coordinated with Alex.
- The distance to production and the plan: figaf-faid `docs/SOLUTION.md` section 1.
