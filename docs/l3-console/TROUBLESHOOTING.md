...
| `cf start figaf-l3l4-backend failed …` and `cf logs figaf-l3l4-backend --recent` shows `schema migrations FAILED: relation "b2b_archiving_configs" already exists` | The database still holds the table that release 0.4.1 created at runtime. From release 0.4.4 migration 0001 creates it and refuses to run over the old one (figaf-l3l4 decision 0013, item 6: fail closed; nobody but Figaf has a 0.4.1 database). | Delete the database instance and let Setup step 3 create it again (`cf unbind-service figaf-l3l4-backend figaf-l3l4-db`, `cf delete-service figaf-l3l4-db -f`, ~8 + ~7 minutes), or a full wipe. Then Install again. |
